...
async def test_get_link_url_rejects_unknown_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch)
    device = make_device()
    user = CurrentUser(id=device.user_id, email="reader@example.test")
    db = FakeSession([device])
...
async def test_get_link_url_404_for_other_users_device(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch)
    device = make_device()
...
async def test_get_link_url_503_when_dropbox_not_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch, dropbox_client_id=None)
    device = make_device()
    user = CurrentUser(id=device.user_id, email="reader@example.test")
    db = FakeSession([device])
...
async def test_get_link_url_503_without_fernet_key(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch, fernet_key=None)
    device = make_device()
    user = CurrentUser(id=device.user_id, email="reader@example.test")
    db = FakeSession([device])
...
async def test_link_callback_stores_encrypted_dropbox_token(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch)
    ...
    assert cloud_links.parse_link_ref(device.link_ref) == {"provider": "dropbox", "token": "dbx-access-tok"}
    assert out.cloud_linked is True
    assert out.cloud_provider == "dropbox"
    assert "dbx-access-tok" not in out.model_dump_json()


async def test_link_callback_stores_encrypted_drive_refresh_token(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch)
    ...
    assert device.link_ref is not None
    assert "goog-refresh" not in device.link_ref
    assert cloud_links.parse_link_ref(device.link_ref) == {
        "provider": "drive",
...
def test_get_devices_http_does_not_leak_dropbox_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    device = make_device(
        name="Ma Kobo",
        link_ref=cloud_links.serialize_link_ref({"provider": "dropbox", "token": token_value}),
    )
...
async def test_list_devices_undecryptable_link_ref_is_unlinked(monkeypatch: pytest.MonkeyPatch) -> None:
    """Cle perdue / jeton corrompu → cloud_linked=false, pas d'exception."""
    foreign_key = Fernet.generate_key().decode()
    encrypted = Fernet(foreign_key.encode()).encrypt(
        json.dumps({"provider": "dropbox", "token": "x"}).encode()
    ).decode()
    _patch_settings(monkeypatch)  # cle de lecture differente de foreign_key
    device = make_device(link_ref=encrypted)
    user = CurrentUser(id=device.user_id, email="reader@example.test")
    db = FakeSession([[device]])
...
def test_link_callback_get_rejects_missing_state(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app.dependency_overrides[__import__("ferry_agent.db", fromlist=["get_db"]).get_db] = fake_db
    try:
...
def test_link_callback_get_rejects_invalid_state(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_settings(monkeypatch)
...
def test_link_callback_get_exchanges_and_redirects(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_exchange(code, client_id, client_secret, redirect_uri):
        assert code == "auth-code"
        return {"access_token": "dbx-access-tok"}
